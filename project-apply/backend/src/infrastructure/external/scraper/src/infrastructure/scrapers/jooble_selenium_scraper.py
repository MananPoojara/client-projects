...
class JoobleSeleniumScraper(BaseScraper):
    """Jooble scraper using Selenium with stealth mode."""
    ...
    def scrape(self, keywords: list[str], location: str = "") -> list[dict]:
        ...
        for keyword in keywords:
            try:
                search_url = f"https://jooble.org/jobs/{keyword.replace(' ', '-')}"
                if location:
                ...
                if not job_cards:
                    job_cards = soup.find_all('div', class_='_1mMWX')
...
def main():
    print("=" * 70)
    print("JOOBLE SCRAPER (Selenium + Stealth)")
    print("=" * 70)
    
    scraper = JoobleSeleniumScraper()
